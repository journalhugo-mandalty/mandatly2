...
// IGN Géoplateforme — parcelles cadastrales (gratuit, sans clé)
...
export default function MapComponent({
  ...
  useEffect(() => {
    ...
    scr.onload = () => {
      ...
      // Clic sur la carte → fetch parcelle IGN au point cliqué
      map.on("click", async (e: any) => {
        if (!layersRef.current?.parcelles) return;
        ...
        try {
          ...
          if (!feat) return;
          const matching = prospectsRef.current.filter(p =>
            p.lat && p.lng && Math.abs(p.lat - lat) < 0.007 && Math.abs(p.lng - lng) < 0.007
          );
          onParcelRef.current?.({ properties: feat.properties, centroid: [lat, lng], matchingProspects: matching });
        } catch {}
      });
  ...
  // ── Couche cadastre IGN (tile raster) ─────────────────────────────────────
  useEffect(() => {
    if (!ready || !mapInst.current) return;
    const L = (window as any).L;
    if (layers?.parcelles) {
      if (!cadastreRef.current) {
        cadastreRef.current = L.tileLayer(TILE_CADASTRE, { opacity:0.7, maxZoom:20 }).addTo(mapInst.current);
  ...
  // ── Helpers layers géographiques (dept/commune) ───────────────────────────
  ...
  // ── Refresh : choix dept / communes selon zoom ────────────────────────────
  const refresh = async () => {
    const map = mapInst.current; if (!map || !(window as any).L) return;
    const z = map.getZoom();
    if (z >= Z_PARCEL) {
      if (deptLayerRef.current) { deptLayerRef.current.remove(); deptLayerRef.current = null; }
      if (commLayerRef.current) { commLayerRef.current.remove(); commLayerRef.current = null; }
  ...
  useEffect(() => { if (!ready) return; refresh(); }, [layers?.ventes, layers?.dpe]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <>
      ...
      <div style={{position:"relative",width:"100%",height:"100%"}}>
        ...
        <div style={{position:"absolute",bottom:40,right:48,zIndex:1000,background:"rgba(10,16,28,0.75)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:6,padding:"3px 8px",fontSize:9,color:"rgba(255,255,255,0.5)",fontWeight:600,backdropFilter:"blur(6px)",letterSpacing:"0.05em"}}>
          {zoom < Z_COMMUNE ? "DÉPARTEMENTS" : zoom < Z_PARCEL ? "COMMUNES" : "PARCELLES IGN"}
        </div>
